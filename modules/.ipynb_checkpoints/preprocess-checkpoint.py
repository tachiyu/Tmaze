...
from modules.Caiman import cell_idx
...
def bdata_process(raw_behavior_root, processed_root, task="", name="", day=""):
    ...
    bdata = pd.read_csv(list((Path(raw_behavior_root)/name/task/day).glob('*.csv'))[0], delimiter=';')
    ...
    for i in range(len(tristarts)-1):
        ...
        bins += list(bin1+bin2)
        t += list((gbdata.DateTime - gbdata.iloc[0].DateTime) * 1000* 60 * 60 * 24)
...
def comb(processed_root, task="", name="", day=""):
    # loading data
